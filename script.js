...
// Add the new HistogramSeries for volume here
const volumeSeries = chart.addSeries(HistogramSeries, {
    color: '#26a69a', // Example color, can be dynamic later
    priceFormat: {
        type: 'volume',
    },
    priceScaleId: '', // Set as an overlay
    // Ensure this series is also on pane 1
}, 1);
...
candlestickSeries.priceScale().applyOptions({
    scaleMargins: {
        top: 0.1, // Example: 10% from the top of its allocated space in the pane
        bottom: 0.3, // Example: 30% from the bottom, leaving space for volume
    },
});

// NEW: Generate and set volume data
const volumeData = candlestickData.map(candle => ({
    time: candle.time,
    value: Math.floor(Math.random() * 29000000) + 1000000, // Example random volume
    color: candle.close >= candle.open ? '#26a69a' : '#ef5350',
}));
volumeSeries.setData(volumeData);
volumeSeries.priceScale().applyOptions({
    scaleMargins: {
        top: 0.7,  // Example: Starts 70% from the top of its allocated space
        bottom: 0, // Takes the remaining space to the bottom
    },
});
